...
\begin{abstract}
\small
A sealed type-theoretic extension---a new modality, universe former, or promoted interface package---must still interact correctly with everything the surrounding library has already exported. We ask how much of that interaction must persist as primitive public trace once the extension is sealed and normalized. The motivating picture is small. Suppose the old interface exports two generators $A,B$ together with an equivalence $p:A\simeq B$, and a new layer exports an operation $X$ acting globally. Specifying $X$'s action on $A$ and on $B$ separately is not enough: the constant action on the Boolean swap is well-formed pointwise but fails the naturality square along $p$, while the identity action passes. At least binary trace is therefore forced. One then worries that comparing further previously exported squares forces ternary, quaternary, and unbounded higher trace. This paper proves that for one fixed cubical sealed-extension calculus $\Cext$, the tower stabilizes at depth two. Realized obligation objects satisfy $\Real_k(X)\simeq\Real_2(X)$ for $k\geq2$, because the apparent higher obligations are open-box extension types whose missing remote face is canonically supplied by cubical composition and filling; above-binary primitive trace is eliminated from $\mu$-minimal public signatures; and the swap obstruction shows that unary trace alone does not decide sealing. A separate recent-history theorem turns the arity bound into a two-step chronological window. The counting results are deliberately corollaries of this trace decomposition: sparse footprints stay sparse, while the fully coupled, bundled per-site, constant-payload endpoint specializes to a shifted Fibonacci recurrence. The accompanying Cubical Agda development checks the fixed-calculus theorem, a \texttt{Partial}/\texttt{Sub}-indexed open-box boundary package with \texttt{hcomp}/\texttt{hfill} contraction, a concrete \texttt{SMod} surface-adequacy instance, recurrence arithmetic, lower-bound witnesses, and trace-accounting discipline; broader transfer from arbitrary cubical surface calculi to $\Cext$ and the fully fibrant path-through-compatible-elements reading remain explicit adequacy assumptions.
\end{abstract}
...
\subsection{Sealed extension creates public trace debt}
...
\begin{quote}
When a type-theoretic core calculus is extended by sealing a new structure against the existing interface, how much public integration trace must survive normalization and opacity?
\end{quote}

The question is not about ordinary transparent user developments inside a fixed library. It is about \emph{sealed} extensions: new type formers, modalities, or promoted interface packages whose public behavior becomes part of the global exported interface. More specifically, the paper studies the worst-case regime of \emph{foundational core extensions}: globally acting additions whose exported payload must be specified across the currently active interface. Examples include extending a univalent universe hierarchy, adding a global modality presented by explicit modal dependent-type syntax~\cite{birkedal-modal-dtt,aagaard-cubical-mtt}, or promoting an interface package whose action is intended to range over the whole library.

Orthogonal extensions are not the subject of the Fibonacci envelope. Adjoining a new datatype such as \texttt{Tree} to a library that already contains \texttt{List} and \texttt{Socket} need not introduce primitive \texttt{Tree}--\texttt{Socket} interaction data merely to preserve canonicity. Such sparse or local extensions are important in practice, but they do not exhibit the maximal coupling pattern isolated here. Ordinary user-level library growth therefore forms a sparse dependency DAG in general; the recurrence theorem studies the foundational-core regime of monolithic, globally acting sealed extensions.
...
\subsection{A running example before the definitions}
\label{subsec:running-example}

Keep the following picture in mind. The old interface exports two generators $A$ and $B$, together with a path or equivalence $p:A\simeq B$. A new sealed layer exports a globally acting operation $X$.
...
\begin{tikzpicture}[baseline=(x.base)]
  ...
  \node (a) at (-1.6,-1.2) {$A$};
  \node (b) at (1.6,-1.2) {$B$};
  \draw[->] (x) -- node[left] {unary trace} (a);
  \draw[->] (x) -- node[right] {unary trace} (b);
  \draw[->] (a) -- node[below] {$p$} (b);
\end{tikzpicture}
\]

The \emph{payload} is the genuinely new operation $X$. The \emph{unary trace} says how $X$ acts on individual old generators such as $A$ and $B$. The \emph{binary trace} says that this action respects old comparison data such as $p$. An apparent ternary obligation asks for compatibility among several already exported squares. In the fixed cubical calculus, that higher structural obligation is represented as a horn-extension package: it is present in the exact object, absent from the primitive public count. Cubical composition and filling canonically supply the missing open-box face and its filler from the displayed partial boundary, so the sealed public signature counts only the primitive payload plus primitive unary and binary trace.

\begin{figure}[t]
\centering
\[
\begin{array}{c|c|c}
\text{arity} & \text{visible structural shape} & \text{counting status} \\
\hline
1 & X\to A & \text{primitive unary trace may survive} \\
2 & (X\to A)\xRightarrow{p}(X\to B) & \text{primitive binary trace may survive} \\
3 & \partial\Box\ \text{with one remote face} & \text{derived horn package in }\Cext
\end{array}
\]
\caption{Unary, binary, and apparent ternary obligations in the running example. Arity counts how many layers of prior public structure the new sealed payload must be compared against; dimension is the corresponding comparison cell. The theorem says that the third row remains present as an exact horn-extension factor, but not as primitive public trace in a $\mu$-minimal signature.}
\label{fig:arity-obligations}
\end{figure}

The Boolean swap example gives the lower bound. If the old interface exports the two-point type and the univalent path induced by swapping its two points, then a constant-left unary action is well-formed objectwise but fails the binary naturality check along the swap path. By contrast, the identity action passes that check. Thus unary trace alone does not decide whether a sealed action is admissible.

\subsection{Guiding conjectures}
\label{subsec:guiding-conjectures}

The paper proves a depth-two theorem for a fixed formal sealed-extension calculus and proposes it as evidence for a broader cubical depth-two conjecture. The intended claim hierarchy is as follows.

\begin{enumerate}
\item \textbf{Cubical sealed-extension depth two.} Adequate cubical foundations with substitution-stable Kan composition and filling, univalence, and a sealing discipline for globally acting structural extensions should have exact structural integration obligations that stabilize at binary trace, after applying the specified structural horn-computation rules. Unary trace alone should be insufficient in general.
\item \textbf{Adequacy transfer.} A broad class of cubical foundations and surface extension languages should admit a faithful interpretation in the raw sealed-extension calculus of this paper, preserving semantic admissibility, primitive-versus-derived trace status, historical support, and public trace cardinality.
\item \textbf{Global action and active-basis coverage.} For native foundational-core extensions whose operational semantics advertises a total global action on the active interface, canonicity-preserving admissibility should force one primitive interaction clause for each active primitive basis site, up to canonical normalization and derived-field elimination. In the fixed calculus this appears as the full-coupling record used by the recurrence theorem.
\item \textbf{Full-coupling counting envelope.} In cubical foundational-core extension sequences satisfying depth-two public trace, two-layer chronological factorization, full active-basis coupling, and the bundled per-site trace convention, primitive trace cost should satisfy a two-step affine recurrence. The Fibonacci statement is only the constant-payload specialization of that counting corollary.
\item \textbf{Two-layer chronological Markov blanket.} Depth as arity does not by itself imply a time window. The recurrence also depends on the conjectural principle that primitive public comparisons with remote history factor through the normalized public signatures of the two most recent layers.
\item \textbf{Horn-computational universality.} The depth-two trace theorem should not be specific to one CCHM presentation, but to foundations whose structural integration obligations are generated as horn-extension packages with substitution-stable computable filling and a nontrivial univalent path structure for the binary lower bound.
\end{enumerate}

Only the fixed-calculus instance is claimed as a theorem in this paper. The transfer from arbitrary Cubical Agda programs or from other cubical foundations into that theorem object remains part of the adequacy program. The conjectural word ``adequate'' is not meant to make the broader thesis tautological; the substantive prediction is that concrete systems of independent interest admit the required bridge. The nearest targets are CCHM-style cubical type theory and Cubical Agda, where the present raw calculus was designed to mirror the available Kan operations and opaque module discipline. Cartesian cubical type theories such as the ABCFHL system~\cite{cartesian-cubical}, set-level cubical systems such as XTT~\cite{xtt}, and multimodal cubical type theories~\cite{aagaard-cubical-mtt} look like plausible but harder transfer targets, because each would require its own account of sealing, primitive-versus-derived trace, and factorization-complete public export. Systems whose computation principle lacks the relevant open-box fillers, or whose module boundary exposes hidden derivations rather than opaque signatures, are not expected to satisfy the bridge without changing the formulation.

The key cubical theorem is an open-box theorem, not a closed-boundary theorem.
The higher structural obligations generated by sealing are represented as total
extension spaces for horns: the missing remote face is part of the data. Such
open-box extension spaces are contractible by cubical composition and filling,
naturally in substitution. This is why they remain present in exact realized
obligation objects but disappear from primitive public trace.

\subsection{The theorem proved here}
\label{subsec:theorem-proved-here}

Let $\Cext$ denote the fixed sealed-extension calculus studied below: a CCHM-style cubical core with univalence and Kan composition~\cite{cubical,cchm-hits}, globally acting extension declarations, opaque public signatures, and the raw adequacy bridge of \autoref{sec:bridge}. The definitions of $\Cext$ are part of the theorem object. They are not claimed to be the unique possible formalization of cubical sealed extension.

\begin{quote}
\textbf{Guiding conjecture.}
Adequate cubical sealed-extension systems have primitive public structural trace depth two. Unary trace is insufficient because univalence supplies genuine binary naturality obligations, while higher structural integration obligations are horn-computationally derived from binary boundary trace.

\textbf{Theorem proved here.}
The guiding conjecture holds for $\Cext$. For every admissible candidate $X$, the realized structural obligation objects satisfy $\Real_k(X)\simeq\Real_2(X)$ for all $k\geq2$; every $\mu$-minimal normalized public signature has no primitive trace field of arity greater than two; and the swap obstruction shows that arity one is insufficient as an admissibility-decision principle.

\textbf{Counting corollary.}
If, in addition, a sealed sequence is fully coupled to the active primitive basis and its binary trace satisfies the recent-history factorization theorem, then primitive trace cost satisfies
\[
  \mu_{n+1} = \mu_n + \mu_{n-1} + \kappa_n + \kappa_{n-1}.
\]
If $\kappa_n=c$, then $U_n=\mu_n+2c$ is Fibonacci.
\end{quote}

This is the paper's self-description: the paper proves a depth-two theorem for a fixed formal sealed-extension calculus and proposes it as evidence for a broader cubical depth-two conjecture.
...
\midrule
A theorem for the fixed calculus $\Cext$ & A transfer theorem for every cubical foundation \\
Public trace depth at sealed boundaries & Internal homotopy truncation or disappearance of higher paths \\
Higher structural horn packages derived from binary boundary trace & Contractibility of arbitrary higher HoTT filler spaces \\
A full-coupling recurrence envelope & Fibonacci growth for ordinary libraries \\
A mechanized model/audit for the fixed raw bridge & Parser or elaborator adequacy for arbitrary Cubical Agda programs \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Informal proof sketch}
\label{subsec:informal-proof}

The upper bound is a horn-filling argument. Once the new layer has unary action on old generators and binary comparison with old paths or equivalences, an apparent depth-three obligation is an open box: all faces except one remote comparison are already visible from the sealed exports of recent layers. In $\Cext$, the marginal higher obligation is represented by the dependent sum of that missing face and a filler. Cubical composition and filling compute this package from the boundary, so it is present in the exact object, absent from the primitive public count. Iterating this argument gives no new primitive public trace above binary arity.
...
The recurrence is then arithmetic. A fully coupled layer must expose primitive trace against every primitive basis site in the previous two public interfaces. The sizes of those interfaces are $\kappa_n+\mu_n$ and $\kappa_{n-1}+\mu_{n-1}$, giving the affine recurrence. Sparse footprints keep only the selected sites, so sparse or orthogonal extension sequences remain sparse.
...
\begin{enumerate}
\item It formulates a sealed-extension calculus for globally acting cubical-style structural extensions, with opaque public signatures and a presentation-invariant count of primitive public trace.
\item It proves, in this fixed calculus, that structural integration obligations above binary arity are horn-computationally derived: $\Real_k(X)\simeq\Real_2(X)$ for $k\geq2$, and $\mu$-minimal public signatures contain no primitive trace fields of arity greater than two.
\item It proves a binary lower bound from univalence using the two-point swap obstruction, showing that unary objectwise trace is insufficient to decide sealing.
\item As a counting corollary, it proves the sparse footprint recurrence and its fully coupled affine endpoint; under the additional constant-payload specialization, the shifted full-coupling sequence is Fibonacci.
\end{enumerate}
...
\begin{table}[t]
\centering
\caption{Short dependency map for the depth-two theorem stack.}
\label{tab:intro-dependency}
\small
...
3 & Horn computation & Makes depth $>2$ derived or contractible \\
...
6 & Counting theorem & Gives sparse and full-coupling recurrence laws \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\autoref{thm:upper}, \autoref{cor:contractible-factor} & exact $\Real$-stabilization at depth~$2$ and trivialization of higher exact fibers & horn reduction and contractible theorem-facing horn fibers & fully formalized \\
...
\autoref{thm:adjunction}, with \autoref{thm:clutching} as an example & lower-bound binary obstruction and topological illustration & swap barrier for the load-bearing lower bound; clutching for a checked geometric sanity test & fully formalized \\
\autoref{thm:recurrence} & affine recurrence for $\mu$ & \autoref{thm:trace}, \autoref{cor:mu-delta}, the sparse/full recurrence package, full coupling, and the chronological-window package & formalized with bridge \\
...
The definitions in this section are part of the theorem object. They are not claimed to be the unique possible formalization of cubical sealed extension. The broader adequacy question is whether a different cubical surface language or semantic foundation admits a faithful transfer into this raw calculus while preserving admissibility, primitive-versus-derived trace status, historical support, and public cardinality.

\paragraph*{Reading guide.}
The running example of \autoref{subsec:running-example} threads through every construction below. With old generators $A,B$, equivalence $p:A\simeq B$, and new sealed operation $X$: the raw syntax of \autoref{subsec:raw-syntax} records the declaration of $X$ together with its unary action clauses on $A$ and $B$ and its binary comparison clause along $p$; the state and sealing definitions formalize the chronology in which $A,B,p$ are exported first and $X$ is then exported sealed; the candidate and counting-normal-form definitions (\autoref{def:candidate}--\autoref{def:atomic-schema}) make precise what it means to count the unary fields $X\to A$ and $X\to B$ and the $p$-naturality square as primitive trace, while ignoring transparently synthesized higher data; \S2.4 (global action and basis coverage) generalizes from the two-site picture $\{A,B\}$ to the requirement that $X$ act on every active primitive site; and the trace decomposition principle (\autoref{thm:trace}) is the structural statement that underwrites the later Fibonacci recurrence. A reader who wants the headline first may skim \autoref{tab:notation} and \autoref{thm:trace}, then return to the intermediate definitions as needed.

\subsection{Notation and terminology}
\label{subsec:notation-terminology}

The paper uses the following notation consistently. The point of the table is
to keep three levels separate: exact realized obligations, normalized public
syntax, and chronological counting.
...
\label{tab:notation}
\small
\begin{tabular}{@{}p{0.18\linewidth}p{0.56\linewidth}p{0.17\linewidth}@{}}
\toprule
Symbol & Meaning & Level \\
\midrule
$\Cext$ & fixed sealed-extension calculus studied in this paper & theorem object \\
$X$ & candidate sealed extension before export & model \\
$L_n$ & sealed layer exported at stage~$n$ & chronology \\
$\kappa_n$ & payload size of layer~$n$ & count \\
$\mu_n$ & primitive public trace size of layer~$n$ & count \\
$\Real_k(X)$ & realized obligation object after exposing depth-$k$ history & exact obligation \\
$\Prim_k(X)$ & primitive part of the normalized obligation signature & public syntax \\
$\Der_k(X)$ & derived part of the normalized obligation signature & public syntax \\
$d_{\mathrm{obl}}$ & exact obligation stabilization depth & metatheory \\
$d_\mu$ & minimal public-signature depth & syntax/counting \\
$\Foot_n$ & active footprint touched by the new layer & recurrence \\
\bottomrule
\end{tabular}
\end{table}
...
Primitive trace & trace that is not synthesized from other public data under the counting normalization \\
Derived trace & trace present in some presentation but computed from lower trace or generic cubical operations \\
...
In the running example of \autoref{subsec:running-example}, the declaration of $X$ has $\Gamma_{\mathrm{new}}$ containing the symbol $X$ alone; $\Gamma_{\mathrm{act}}$ contains one unary action field for the site $A$ and one for $B$; $\Gamma_{\mathrm{cmp}}$ contains the single binary naturality clause along $p$; and the export selection $\sigma$ keeps all four. Higher-arity boundaries among further previously exported squares would, if naively recorded, appear as additional fields of $\Gamma_{\mathrm{cmp}}$ in raw horn shape; \autoref{sec:coherence} will show that they do not survive in a $\mu$-minimal opaque signature.
...
they are not raw \emph{derived} syntax. The raw grammar records only that the
clause has horn shape; derived public status is assigned later, and only when a
derivation witness synthesizes the field from the lower boundary using the
...
An \emph{atomic schema} is one field of this counting normal form after transparent telescope isomorphisms have been quotiented. Such a field is assigned derived public status when its inhabitant is uniformly synthesized from earlier fields by definitional equality, reflexivity, transport along already exported equalities, or the generic structural operations of the ambient calculus. It is assigned primitive status when no such transparent synthesis is available in the fixed calculus. The cost and basis notions below count primitive atomic schemas; derived schemas may remain present in exact signatures but contribute no primitive public trace unit.
\end{defi}
...
\begin{quote}
\small
\textbf{What this result says.}
In the fixed native-totality record, a fully coupled declaration has one
primitive interaction site for each active primitive basis site after counting
normalization.
\textbf{What it does not say.}
It does not derive full coupling from ordinary canonicity for arbitrary local
extensions.
\textbf{Where the assumption enters.}
Full coupling is the footprint hypothesis of \autoref{def:footprint}; the
coverage theorem then counts the irreducible public fields required to realize
that hypothesis.
\end{quote}
...
normalization relation preserves primitive/derived public statuses, transparent-generation
...
\[
  N(L_k) \cong N_{\mathrm{core}}(L_k) \uplus N_{\mathrm{tr}}(L_k).
\]
Primitive and derived public statuses are part of the counting normal form, so the same
...
\small
\textbf{What this result says.}
The bridge identifies which fields of a raw sealed declaration become payload,
which become primitive public trace, and which are derived before $\mu$ is
computed.
\textbf{What it does not say.}
It is not a parser or elaborator theorem for arbitrary Cubical Agda programs.
\textbf{Where the assumption enters.}
All downstream semantic readings pass through the adequacy boundary recorded in
\autoref{rem:semantic-adequacy-package} and the fixed raw bridge of
\autoref{rem:bridge-target}.
\end{quote}
...
\begin{tikzpicture}[
  ...
  longlabel/.style={
    ...
    text width=2.8cm,
    inner sep=1pt}]
  \node[box] (sem) at (-6.0,0) {semantic\\sealed\\extension};
  \node[box] (adq) at (-3.0,0) {adequacy\\package};
  \node[box] (raw) at (0,0) {raw\\declaration};
  \node[box] (norm) at (3.0,0) {normalized\\public\\signature};
  \node[box] (mu) at (6.0,0) {$\mu$\\primitive\\trace};
  \draw[->] (sem) -- node[flowlabel, above] {transfer} (adq);
  \draw[->] (adq) -- node[flowlabel, above] {raw\\image} (raw);
  \draw[->] (raw) -- node[flowlabel, above] {seal\\normalize} (norm);
  \draw[->] (norm) -- node[flowlabel, above] {count} (mu);
  \draw[->,dashed] (adq.south) to[bend right=18]
    node[longlabel, below=2pt] {preserve support\\and derived status}
    (norm.south);
\end{tikzpicture}
\caption{The adequacy boundary. A semantic sealed extension is counted only after an adequacy package interprets it in the fixed raw calculus, normalization exposes the sealed public signature, and $\mu$ counts the primitive trace fields that remain after derived horn data are removed.}
\label{fig:adequacy-flow}
\end{figure}

\subsection{What an adequacy theorem would have to prove}
\label{subsec:adequacy-preservation}

The internal theorem is conditional on exactly the following preservation
properties. An adequacy theorem from a richer cubical surface language into
$\Cext$ must preserve semantic admissibility, primitive-versus-derived trace
status, historical support, public cardinality~$\mu$, the horn-extension
representation of structural integration obligations, the correspondence
between opacity/sealing and the intended semantic boundary, and the
correspondence between advertised global action and full-coupling coverage.
This makes the conjecture falsifiable: a counterexample must either violate
one of these bridge clauses or exhibit a structural obligation not representable
as a derived horn-extension package in the target calculus.
...
\subsection{A concrete adequacy instance: global modality over a Tarski universe}
\label{subsec:smod-adequacy}

To make the adequacy boundary non-vacuous, the artifact includes a small surface calculus, \emph{\texttt{SMod}}, for adding a global modality over an opaque old Tarski universe. This is the running example of \autoref{subsec:running-example} concretized: the role of the old generators $A,B$ is played by codes in the old Tarski universe $U$, the equivalence $p:A\simeq B$ is replaced by the structural equivalence $\mathsf{ElArr}$ between code-formed and meta-formed arrows, and the new sealed operation $X$ is the global modality \texttt{Flat} together with its decoding traces $\mathsf{flatEl}$ and $\mathsf{flatCodeArr}$. The two routes around the function-code square below reproduce the binary naturality picture of the running example, and the comparison between them is the higher horn that the depth-two theorem will eliminate from $\mu$. The old sealed interface is
\[
\begin{array}{rcl}
  U&:&\Type,\\
  \El&:&U\to\Type,\\
  ...
  \mathsf{ElArr}&:&(A\,B:U)\to
    \El(\mathsf{Arr}(A,B))=(\El(A)\to\El(B)).
\end{array}
\]
The new declaration has payload
\[
  \mathsf{Flat}:\Type\to\Type,
  \qquad
  \eta:(A:\Type)\to A\to\mathsf{Flat}(A),
\]
and primitive structural trace
\[
\begin{array}{rcl}
  \mathsf{FlatU}&:&U\to U,\\
  \mathsf{flatEl}&:&(A:U)\to
    \El(\mathsf{FlatU}(A))=\mathsf{Flat}(\El(A)),\\
  \mathsf{flatCodeArr}&:&(A\,B:U)\to
    \mathsf{FlatU}(\mathsf{Arr}(A,B))=
    \mathsf{Arr}(\mathsf{FlatU}(A),\mathsf{FlatU}(B)),\\
  \mathsf{flatArr}&:&(A\,B:U)\to
    \mathsf{Flat}(\El(A)\to\El(B))=
    (\mathsf{Flat}(\El(A))\to\mathsf{Flat}(\El(B))).
\end{array}
\]
For each $A,B:U$ there are two routes from
\[
  \El(\mathsf{FlatU}(\mathsf{Arr}(A,B)))
  \quad\text{to}\quad
  \mathsf{Flat}(\El(A))\to\mathsf{Flat}(\El(B)).
\]
The left route applies \(\mathsf{flatEl}\) at \(\mathsf{Arr}(A,B)\), maps
\(\mathsf{ElArr}\) through \(\mathsf{Flat}\), and then applies
\(\mathsf{flatArr}\). The right route applies \(\mathsf{flatCodeArr}\), decodes
with \(\mathsf{ElArr}\) at the lifted codes, and transports domain and codomain
along \(\mathsf{flatEl}(A)\) and \(\mathsf{flatEl}(B)\). The comparison between
these routes is the generated higher horn \(\mathsf{flatArrCoh}\). A surface
program may write it explicitly, but it is accepted as derived trace only
after the route boundary has produced the horn derivation witness. It therefore
does not contribute to~\(\mu\).

\begin{center}
\small
\begin{tabular}{@{}p{0.24\linewidth}p{0.22\linewidth}p{0.18\linewidth}p{0.22\linewidth}@{}}
\toprule
Surface field & Raw clause class & Primitive? & Counted by $\mu$? \\
\midrule
\texttt{Flat}, \texttt{eta} & payload & payload & no \\
\texttt{FlatU}, \texttt{flatEl} & action/decoding trace & primitive & yes \\
\texttt{flatCodeArr}, \texttt{flatArr} & comparison trace & primitive & yes \\
\texttt{flatArrCoh} & horn clause & derived & no \\
\bottomrule
\end{tabular}
\end{center}

\begin{thm}[Modal surface adequacy]
\label{thm:modal-surface-adequacy}
Let $E$ be a well-typed declaration in \texttt{SMod} over an opaque old
universe interface. Then \(\mathsf{Elab}(E)\) is an admissible raw
\(\Cext\)-declaration. Elaboration preserves payload fields, primitive trace
fields, historical support, and normalized public cost~\(\mu\). The generated
function-code compatibility horn elaborates to a structural horn clause
together with a derivation witness for its derived replacement; adding it
explicitly as a surface field does not change the normalized public signature
or~\(\mu\).
\end{thm}

\begin{proof}
The proof is by inspection of the syntax-directed typing derivation. Payload
fields map to raw payload clauses. \(\mathsf{FlatU}\) and
\(\mathsf{flatEl}\) map to unary action/decoding clauses, while
\(\mathsf{flatCodeArr}\) and \(\mathsf{flatArr}\) map to binary comparison
clauses. The route comparison has exactly the structural open-box shape
described in \autoref{thm:actual-open-box-contractibility}; if it is omitted,
elaboration inserts the canonical center, and if it is supplied explicitly,
contractibility identifies it with the canonical witness. In both cases the
horn derivation supplies the derived replacement term. The normalized surface
telescope is therefore canonically isomorphic to the normalized raw telescope
with the same primitive fields and witnessed derived fields, so support and
\(\mu\) are preserved.

The mechanized instance lives in
...
\AgdaPath{Surface/Modal/Normalization.agda}, and
\AgdaPath{Surface/Modal/Adequacy.agda}; the smoke test
\AgdaPath{Test/Surface/ModalAdequacySmoke.agda} checks the minimal
declaration, the explicit-horn presentation through
\AgdaPath{modal-explicit-horn-elaborates-through-HornElaboration}, the
omitted-horn insertion of the same derived term, and rebundling invariance
of~\(\mu\).
\end{proof}
...
\label{rem:transparency-boundary}
The phrase ``generated transparently'' is a theorem-facing boundary, not a
license to declare inconvenient fields derived. A field is transparent only
when the fixed derivability relation exhibits a uniform term for it from
already exported lower data, using definitional equality, record/telescope
isomorphisms, transport along exported equalities, or the cubical Kan
operations named in the raw bridge. Changing that relation would indeed change
the measured primitive signature, which is why the paper treats it as part of
the adequacy boundary rather than as informal notation. The two-layer window
therefore depends on a checkable claim: the remote comparison field must be
computed by the horn term supplied by \autoref{lem:horn-reduction} and
\autoref{thm:computational-replacement}. If a proposed cubical foundation
cannot supply such a uniform term, its remote field remains primitive and the
foundation does not instantiate the depth-two bridge.
\end{rem}
...
\autoref{thm:higher-elim}. In the artifact this is exposed by
\AgdaPath{RawTraceField}, \AgdaPath{PublicStatus}, and
\AgdaPath{SemanticTraceNormalFormWithDerivation}: unary and binary roles remain
primitive candidates only through an irreducibility witness, while a horn role
must carry a \AgdaPath{HornSemanticDerivation} with an explicit
\AgdaPath{StructuralOpenBox} and \AgdaPath{openBoxCenter} producing the
corresponding semantic derived horn trace.
...
\autoref{thm:adjunction}, \autoref{cor:d2}; \autoref{thm:clutching} as example
& lower bound and exact depth & Binary sealing data are necessary in the
univalent cubical setting, higher exact factors stabilize, and clutching
illustrates the pattern without carrying the proof. \\
...
\small
\textbf{What this result says.}
In a UIP setting, higher exact equality obligations collapse to unary
boundary data, so obligation depth is one.
\textbf{What it does not say.}
It is not the cubical theorem and does not describe univalent binary trace.
\textbf{Where the assumption enters.}
The collapse uses UIP directly, through uniqueness of identity proofs between
fixed endpoints.
\end{quote}
...
\small
\textbf{What this result says.}
In $\Cext$, higher structural integration obligations generated by the sealing
grammar are represented as horn-extension packages, become derived public trace
by cubical computation, and leave exact public trace depth two.
\textbf{What it does not say.}
It does not claim that arbitrary higher homotopy types, user-supplied higher
payload, or general cubical filler spaces are contractible.
\textbf{Where the assumption enters.}
The horn-package assumption enters through the structural-integration grammar
and \autoref{def:horn-extension-package}; Kan composition and filling supply
the computational replacement.
\end{quote}

From now on we specialize the paper's fixed theorem object to the cubical extension calculus built over the chosen CCHM-style core~\cite{cubical,cchm-hits}, with univalence and Kan composition available in the ambient operational semantics. Full coupling is not used to state the exact-depth theorem; it is reintroduced in \autoref{sec:scaling} when the two-layer window is counted as an equality-style recurrence. The next ingredients are deliberately complementary: an exact stabilization theorem for realized obligation objects gives the bound $d_{\mathrm{obl}} \leq 2$; \autoref{thm:higher-elim} gives the separate syntactic bound $d_\mu \leq 2$ on $\mu$-minimal public presentations; recent-history factorization turns the exact depth bound into a genuine two-step time window; and the explicit swap obstruction gives the load-bearing lower bound. Together they determine exact obligation depth~$2$.

Univalence and cubical computation play different roles in that argument. Univalence supplies the lower-bound source of genuine binary coherence, witnessed below by the swap obstruction. The clutching discussion is an additional topological example of the same unary/binary separation, not a second load-bearing proof of the upper bound. Univalence does not by itself remove primitive arity-$3$ trace fields or produce exact stabilization of realized obligation objects. Those exact-stabilization and elimination statements use the separate cubical horn-computation package together with recent-history factorization.
...
\paragraph*{The running example, before the formalism.}
Recall the picture of \autoref{subsec:running-example}: old generators $A,B$, equivalence $p:A\simeq B$, and a new sealed operation $X$ acting globally. Suppose a third old generator $C$ is exposed at depth three, together with an already-exported square $q$ comparing $A$ and $C$ through $B$. A naively recorded depth-three obligation would ask for a comparison among the action of $X$ on $A$, on $B$, on $C$, and on the squares $p$ and $q$, drawn as a small cube whose six faces are the binary naturality squares already exported. Five of those faces are visible: the two-step compositions through $B$, the boundaries along $p$ and $q$, and the degenerate faces generated by transport. The sixth face---the direct comparison of $X$'s action on $A$ and $C$ across the new layer---is the only piece of new data the cube actually requires. The theorems of this subsection say that, in $\Cext$, this missing face together with its filler is a contractible open-box extension type, and is therefore replaceable by a canonical witness synthesized from the lower-arity boundary by Kan composition. Above-binary structural trace, in this sense, is bookkeeping: the cube is reconstructible from its visible faces. The formal definition below packages this picture as the structural horn-extension object $H_k(\partial)$.
...
\subsubsection{Actual cubical open-box extension theorem}

The preceding definition has two layers. The \emph{structural horn shape} is
the sealing-generated tuple
\[
  (A_k,\phi_k,u_k,u^0_k),
\]
where $A_k$ is the fibrant family in which the integration cell lives,
$\phi_k$ is the visible-face cofibration, $u_k$ is the visible partial
boundary, and $u^0_k$ is the base face compatible with that boundary. The
\emph{structural horn-extension object} is the associated open-box extension
space. The older notation
\[
  \sum_{\gamma:\Gamma^{(k)}_{\mathrm{miss}}(\partial)}
  \mathsf{Fill}^{(k)}_{\partial}(\gamma)
\]
is retained as an intuitive decomposition into missing face plus filler; the
formal object used in the proof is the open-box extension type below.

\begin{lem}[Missing-face packages are open-box extensions]
\label{lem:missing-face-openext}
For every structural horn generated by the sealing grammar, the missing-face
package $H_k(\partial)$ is canonically equivalent to an open-box extension
type
\[
  \OpenExt_\Gamma(A_k,\phi_k,u_k,u^0_k).
\]
The equivalence is natural in cubical substitution.
\end{lem}

\begin{proof}
Expand $\Gamma^{(k)}_{\mathrm{miss}}(\partial)$ as the omitted $t=i1$ face of
the open box. A choice of $\gamma$ together with a filler is the same data as a
lid at $t=i1$ agreeing with the visible partial boundary together with a
relative filler from the base face to that lid. Restricting to faces,
reassociating dependent sums, and reindexing partial elements all commute with
substitution, giving the naturality clause.
\end{proof}

\begin{thm}[Actual structural horn-extension contractibility]
\label{thm:actual-open-box-contractibility}
Let $\Gamma$ be a cubical context and let
$A:\Gamma,t:I\vdash\Type_\ell$ be a fibrant family. Let $\phi$ be the
visible-face cofibration of a structural integration horn generated by the
sealing grammar. In a CCHM presentation $\phi$ is a face-lattice formula; in
Cubical Agda it is represented by an interval expression whose truth predicate
indexes \texttt{Partial}. Assume compatible open-box data
\[
  u:(t:I)\to \Partial{\phi}{A(t)}
  \qquad\text{and}\qquad
  u_0:A(i0)[\,\phi|_{t=i0}\mapsto u(i0)\,].
\]
Define
\[
  \OpenExt_\Gamma(A,\phi,u,u_0)
  :=
  \sum_{\ell_{\mathrm{lid}}:A(i1)[\,\phi|_{t=i1}\mapsto u(i1)\,]}
    \mathsf{Fill}(A,\phi,u,u_0,\ell_{\mathrm{lid}}),
\]
where a filler is a relative section $f:(t:I)\to A(t)$ satisfying
$f(i0)=\mathsf{outS}(u_0)$, $f(i1)=\mathsf{outS}(\ell_{\mathrm{lid}})$, and agreement with
$u(t)$ on $\phi$. Then
\[
  \Gamma\vdash \mathsf{isContr}\bigl(\OpenExt_\Gamma(A,\phi,u,u_0)\bigr).
\]
The center is the lid computed by cubical composition and the filler computed
by cubical filling:
\[
  \bigl(\mathsf{inS}(\compop(A,\phi,u,u_0)),\ \mathsf{hfill}(A,\phi,u,u_0)\bigr).
\]
For every cubical substitution $\sigma:\Delta\to\Gamma$ there is a canonical
equivalence
\[
  \OpenExt_\Gamma(A,\phi,u,u_0)[\sigma]\simeq
  \OpenExt_\Delta(A[\sigma],\phi[\sigma],u[\sigma],u_0[\sigma])
\]
that maps centers to centers, and the contraction paths commute with
substitution propositionally.
\end{thm}

\begin{proof}
The visible structural boundary is encoded as an actual cubical partial
element. In the CCHM presentation the visible faces form a face formula
$\phi$; in Cubical Agda they form an interval expression indexing
\texttt{Partial}. The compatibility equations on overlaps are precisely the
well-typedness conditions for $u$ and the subtype judgment for $u_0$.

By \autoref{lem:missing-face-openext}, the paper's missing-face/filler package
is this open-box extension type. Cubical composition gives the canonical lid,
and cubical filling gives the relative filler. Given another lid/filler pair,
one applies cubical composition one dimension higher in the relative extension
type, with boundary given by the candidate filler, the canonical filler, the
visible partial boundary on $\phi$, and the base face at $t=i0$. The endpoint
and overlap equations are exactly the filler laws for the two extensions, so
the resulting higher filler is a path from the candidate extension to the
canonical one. Uniformity of composition and filling under substitution gives
the naturality statement.
\end{proof}

\begin{rem}[Agda boundary for the open-box theorem]
\label{rem:agda-open-box-boundary}
The mathematical theorem above uses the compatible-element presentation of the
open-box extension. In Cubical Agda, however, the subtype
\texttt{A [ phi -> u ]} lives in \texttt{SSet}, not in the fibrant
\texttt{Type} universe expected by \texttt{PathP}. The mechanization therefore
checks the actual \texttt{Partial}/\texttt{Sub} boundary package as
\AgdaPath{StructuralOpenBox}, \AgdaPath{CompatibleBoundaryFamily}, and
\AgdaPath{CompatibleLid}, exposes the library-specific wrapper
\AgdaPath{canonicalFillSub}, and proves the
homogeneous carrier-level extension contraction \AgdaPath{openExtIsContr}
together with the reusable dependent-singleton lemma
\AgdaPath{isContrPathPSingleton}. The identification of the paper's fully
fibrant compatible-element presentation with this Cubical Agda boundary
package is an explicit adequacy assumption of the fixed calculus, not a hidden
kernel-checked theorem.
\end{rem}

\begin{lem}[Substitution stability of structural open boxes]
\label{lem:openext-substitution-stability}
For every substitution $\sigma:\Delta\to\Gamma$, the reindexed tuple
$(A_k,\phi_k,u_k,u^0_k)[\sigma]$ is the structural open box generated by the
same sealing rule in context~$\Delta$. The canonical center and contraction
of \autoref{thm:actual-open-box-contractibility} are stable under this
reindexing.
\end{lem}

\begin{proof}
Substitution acts structurally on interval expressions, face formulas, partial
elements, and subtypes. CCHM composition is uniform under substitutions, and
Cubical Agda's primitive composition/filling operations have the corresponding
stability in the type checker. The Agda development records this as the
parametric reindexing API \AgdaPath{CubicalOpenBox/Substitution.agda}.
\end{proof}

\begin{rem}[Open boxes are not closed boundaries]
\label{rem:open-box-not-closed-boundary}
\leavevmode\par\smallskip
\noindent\fbox{\begin{minipage}{0.9\linewidth}
\textbf{Scope.} H1 is an open-box extension theorem. It does not assert
contractibility of arbitrary closed-boundary filler spaces.
\end{minipage}}
\par\smallskip

This theorem is an open-box theorem. It does not say that the type of fillers
for a closed boundary, such as a full sphere boundary, is contractible. For a
closed boundary the filler fiber can carry genuine homotopy. The contractibility
used here comes from the fact that the missing remote face is part of the
extension data. Equivalently, the inclusion is an open-box inclusion, not a
boundary inclusion. The mechanization therefore exposes
\AgdaPath{CubicalOpenBox/Extension.agda} and proves
\AgdaPath{openExtIsContr}; it does not define the theorem-facing extension
fiber as a singleton record, and it includes
\AgdaPath{Test/CubicalOpenBox/ClosedBoundaryNotClaimed.agda} to keep the
closed-boundary case outside this API.
\end{rem}
...
Let $X$ be a foundational core candidate sealed at stage~$n+1$ in the fixed cubical extension calculus. For every $k \geq 3$ and every boundary $\partial : \Real_{k-1}(X)$, the additional data exposed by adjoining the remote layer $L_{n-k+1}$ are canonically represented by a horn-extension fiber
\[
  H_k(\partial)
  :=
  \sum_{\gamma : \Gamma^{(k)}_{\mathrm{miss}}(\partial)}
...
Read top-to-bottom, \texttt{Payload} is the human-authored new content, while \texttt{Trace} is the compatibility debt forced by sealing against the old universe. The two routes are the two composites from \texttt{El (FlatU (Arr A B))} to \texttt{Flat (El A) -> Flat (El B)}. The marginal depth-$3$ obligation is not the space of fillers for a closed boundary. It is the horn-extension type whose free component is the remote comparison between those composites and whose second component is the $3$-cell certifying compatibility with the displayed boundary. In a cubical presentation this total open-box extension type is canonically contractible: composition supplies the canonical missing face and filling packages the accompanying filler. The extra factor is therefore present but contractible, rather than absent.
...
\[
\begin{array}{c}
\Real_3(X)\ \simeq\ \Real_2(X)\times C_3(X),
\qquad C_3(X)\ \text{contractible}
\\[1.0ex]
\downarrow\ \text{canonical replacement through }\Cext
\\[1.0ex]
\Prim(N_{\mathrm{tr}}(e'))=\Prim_{\leq2}(N_{\mathrm{tr}}(e')),
\qquad
\Der(N_{\mathrm{tr}}(e'))\ \text{may still contain horn fields}
\end{array}
\]
\caption{Exact obligations versus the primitive public count. The higher horn factor is part of the exact realized object, displayed here at depth~$3$ as $\Real_3(X)\simeq\Real_2(X)\times C_3(X)$ with $C_3(X)$ contractible. After canonical replacement, the corresponding field is derived, so $\mu$ counts only primitive trace of arity at most two.}
\label{fig:exact-vs-public-count}
\end{figure}

\begin{thm}[Minimal-signature elimination in the fixed calculus]
...
\begin{proof}
Fix any surface presentation of~$e$ and any primitive historical trace clause~$\phi$ of arity $k \geq 3$ occurring in its elaborated sealed export. By \autoref{thm:elaboration}, this clause is part of the public typing judgment before counting normalization, and by \autoref{thm:adequacy} any irreducible contribution it makes would have to survive in the canonical normalized trace signature. By \autoref{lem:horn-reduction}, the additional datum contributed by that arity-$k$ step is a horn-extension fiber $H_k(\partial)$ over a boundary~$\partial$ already exported by lower-arity trace fields. The fixed bridge treats the three generated cases separately: remote-comparison horns provide the missing comparison face, degenerate horns provide transparent identity or composite faces, and transported horns align endpoints along already exported comparison paths. In each case the elaboration package records the cubical family, visible face formula, side boundary, overlap equations, compatible base face, open box, canonical center, decoding map, endpoint/transport equations, and substitution-stability witness. Cubical Kan structure then computes a canonical witness of the fiber from the boundary alone: $\hcomp$ synthesizes the missing remote face, $\transp$ aligns the comparison along the relevant boundary transport, and $\mathsf{hfill}$ packages the resulting higher filler.

Pass from the chosen surface presentation to its canonical normalized trace signature via \autoref{sec:bridge}, normalize the trace telescope in dependency order, and apply \autoref{thm:computational-replacement} to the presented field corresponding to~$\phi$. The theorem shows that within that canonical presentation interface the field is transparently derived from lower-arity boundary data and contributes no irreducible primitive trace schema. Termination is by finiteness of the trace telescope together with the lower-prefix condition on every replacement term: a replacement may use the earlier public boundary and Kan operations, but not the field being removed or any later field. Since any surface declaration contains only finitely many primitive clauses of arity at least~$3$, repeating this reasoning for each such clause yields a presentation-equivalent declaration~$e'$ whose canonical normalized trace signature contains no irreducible historical field of arity~$k \geq 3$. By \autoref{prop:canonical-presentation}, the same canonical normalized trace signature is attached to every representative of that class. Thus $\mu(e)$ counts payload plus irreducible unary and binary public trace, and not the generated higher structural horn fields.
\end{proof}

\begin{cor}
\label{cor:dmu2}
In the fixed cubical extension calculus, the minimal-signature depth satisfies $d_\mu \leq 2$.
\end{cor}
...
\begin{lem}[Kan contraction for structural horn-extension fibers]
\label{lem:kan-horn-contract}
For every structural horn-extension fiber $H_k(\partial)$ generated by
\autoref{lem:horn-reduction} in the fixed cubical calculus, the projection
\[
  H_k(\partial)\to\mathbf{1}
\]
is contractible in the ordinary cubical sense. The center is the missing face
and filler computed from $\partial$ by the Kan operations, and the contraction
is the cubical path supplied by the filler stability equations for open-box
extension problems.
\end{lem}

\begin{proof}
By \autoref{lem:missing-face-openext}, the fiber $H_k(\partial)$ is the
open-box extension type for the structural tuple
$(A_k,\phi_k,u_k,u^0_k)$. Applying
\autoref{thm:actual-open-box-contractibility} gives a canonical center and a
contraction of the whole extension space. No presentation quotient is used to
prove this contractibility; presentation equivalence is used only later, in
\autoref{thm:computational-replacement}, to classify the corresponding public
field as derived. This is the theorem-facing content mechanized as
\AgdaPath{openExtIsContr}, \AgdaPath{horn-extension-fiber-contractible}, and
\AgdaPath{contractible-remote-factor-contractible}.
\end{proof}

\begin{thm}[Exact stabilization of realized structural obligations]
\label{thm:upper}
\label{thm:primitive-depth-upper-bound}
In the fixed cubical extension calculus, for every foundational core candidate~$X$ and every $k \geq 2$ there is a canonical equivalence
\[
  \Real_k(X) \simeq \Real_2(X).
\]
...
\[
  ...
  H_3(\partial).
\]
By \autoref{lem:kan-horn-contract}, this center contracts the fiber in the
cubical metatheory itself: for every $(\gamma,q):H_3(\partial)$ there is a path
in $H_3(\partial)$ from $(\gamma,q)$ to $c_3(\partial)$. This is an absolute
contractibility statement about the theorem-facing horn-extension type, not a
contractibility-up-to-presentation-equivalence statement. Presentation
equivalence enters only after this metatheoretic fact has been established,
when \autoref{thm:computational-replacement} classifies the associated public
field as derived in a $\mu$-minimal signature. Therefore each fiber
$H_3(\partial)$ is contractible. Equivalently, $\Real_3(X)$ is a dependent sum
over $\Real_2(X)$ with contractible fibers, so
\[
  \Real_3(X) \simeq \Real_2(X).
\]

Now let $k>3$. By \autoref{lem:horn-reduction}, passing from depth $k-1$ to depth~$k$ again adjoins only a horn-extension fiber $H_k(\partial)$ over each $\partial : \Real_{k-1}(X)$: one more remote comparison face together with a filler against boundary data already exported by lower layers. \autoref{lem:kan-horn-contract} contracts that fiber. Hence
\[
...
\centering
\small
\begin{tabular}{@{}p{0.22\linewidth}p{0.25\linewidth}p{0.41\linewidth}@{}}
\toprule
Statement & What it bounds & What still needs proof \\
\midrule
$d_\mu\leq2$ & primitive field arity &
remote binary fields might still point far back \\
recent-history factorization &
primitive chronological support &
$L_{n-r}$ for $r\geq2$ factors through $L_n,L_{n-1}$ \\
two-layer window &
counted active basis &
$I_{n,\mathrm{bas}}^{(2)}=S_{\mathrm{bas}}(L_n)\uplus S_{\mathrm{bas}}(L_{n-1})$ \\
\bottomrule
\end{tabular}
\caption{Arity depth versus chronological window. A bound on primitive arity says how many public objects a trace field compares, but not how far back those objects may live in time. The recent-history theorem supplies the extra factorization needed before the recurrence can count only the two most recent sealed layers.}
\label{fig:arity-vs-chronology}
\end{figure}
...
kept as an extended topological example: it shows the same unary/binary
separation in a classical HIT presentation and checks that the fixed
horn-elimination story agrees with a familiar bundle construction. It does not
prove that arbitrary cubical foundations have no primitive arity-$3$
obligations, and the main exact-depth proof does not depend on it.

\begin{rem}[Clutching family example]
\label{thm:clutching}
The HIT clutching presentation over $S^2$ is a topological example of the depth-two pattern: a genuine binary comparison is necessary to specify the bundle data, while this particular HIT presentation asks for no independent primitive arity-$3$ constructor.
...
Crucially, the HIT signature of $S^2$ contains no constructor of dimension~$3$ or higher. Accordingly, the suspension eliminator for this example asks for no primitive input beyond the two point data and the path family~$\gamma$ above. Once the binary path-over-loop has been supplied, any further compatibility condition in this presentation appears only as a horn-extension problem over the already displayed eliminator boundary. By the same cubical filling argument used above, that higher extension type is computed from the boundary and therefore disappears from every $\mu$-minimal public presentation by \autoref{thm:higher-elim}. This is why the clutching example is useful pedagogically, but it is not used as a universal no-arity-$3$ theorem.
\end{rem}

\begin{cor}[Fixed-calculus exact public trace depth two]
\label{cor:d2}
\label{thm:cubical-depth-exactly-two}
In the fixed cubical extension calculus considered above, the primitive public structural trace of admissible sealed structural extensions has exact coherence depth~$2$.
\end{cor}

\begin{proof}
\autoref{thm:upper} provides the exact stabilization statement by showing the realized-obligation equivalence $\Real_k(X) \simeq \Real_2(X)$ for all $k \geq 2$, and \autoref{thm:higher-elim} turns that equivalence into the syntactic statement that no $\mu$-minimal public signature needs primitive arity-$3$ data. \autoref{lem:telescopic} shows that this depth-two exact-stabilization statement is realized by a genuine two-layer chronological window rather than merely by a bound on cell dimension. \autoref{thm:adjunction} gives the load-bearing lower bound by exhibiting a genuine binary sealing obstruction that does not collapse in the univalent setting. The clutching discussion of \autoref{thm:clutching} is an additional topological example of the same pattern, but it is not needed for the exact-depth proof. Therefore the least stabilizing obligation depth is~$2$.
\end{proof}

\section{The complexity scaling theorem}
\label{sec:scaling}

The recurrence theorem is intentionally arithmetic. Its role is to expose the
scaling consequence of the depth-two public-trace theorem once a chronological
window and a coupling footprint have already been fixed.

\begin{quote}
\small
\textbf{What this result says.}
Sparse footprints count only selected active sites; the Fibonacci law appears
only at the full-coupling endpoint of the two-layer window.
\textbf{What it does not say.}
It is not a claim that ordinary libraries, local datatypes, or transparent
definitions grow by Fibonacci recurrence.
\textbf{Where the assumption enters.}
The equality recurrence uses both \autoref{cor:chrono-window} and the
full-footprint judgment $\Foot_{n,\mathrm{full}}^{(2)}=I_{n,\mathrm{bas}}^{(2)}$.
\end{quote}
...
For a concrete picture, suppose the two-layer active interface has primitive
sites
\[
  A,\ B,\ C,\ D,\ E.
\]
A sparse declaration might touch only $A$ and $C$, giving trace cost
proportional to two selected sites. A full-coupling declaration advertised as a
global modality or universe-level operation must touch all five sites, because
its action is part of the exported foundational interface.

\begin{figure}[t]
\centering
\[
\begin{array}{c|ccccc|c}
& A & B & C & D & E & \text{selected sites}
\\
\hline
\text{sparse depth }2
& yes & no & yes & no & no & \{A,C\}
\\[0.6ex]
\text{fully coupled depth }2
& yes & yes & yes & yes & yes & \{A,B,C,D,E\}
\end{array}
\]
\caption{Sparse versus fully coupled footprints at depth~$2$. Sparse growth may select only part of the recent interface, for instance $A$ and $C$, while full coupling selects every active primitive site in the two-layer window. The recurrence is the second row read through the trace-principle identity $|S_{\mathrm{bas}}(L_i)|=\kappa_i+\mu_i$.}
...
The artifact mirrors this split: the semantic full-coupling record provides a
window isomorphism and cardinality law, while the older recurrence calculus
keeps sparse footprints and full footprints as separate predicates before taking
the full-coupling endpoint.
...
\subsection{\texorpdfstring{Affine Fibonacci full-coupling envelope at $d=2$}{Affine Fibonacci full-coupling envelope at d=2}}
...
Let a sealed extension calculus satisfy the following package. Here the fiber
$H_k(\partial)$ is not the ambient space of all semantic fillers for a boundary;
it is the theorem-facing exported horn-extension package selected by the sealed
calculus after normalization.
\begin{enumerate}
  ...
  \]
  \item Every theorem-facing horn-extension fiber~$H_k(\partial)$ of Property~(1) is canonically contractible.
  \item Every admissible surface extension declaration admits a canonical normalized trace presentation, together with presentation equivalence and witnessed primitive-versus-derived public status on its trace fields.
...
\begin{proof}
By Property~(1), each passage from depth $k-1$ to depth~$k$ with $k \geq 3$ decomposes the exact realized obligation object into a depth-$(k-1)$ boundary together with one theorem-facing horn factor. Property~(2) makes that horn factor contractible, so the projection to the boundary is a canonical equivalence. Iterating from depth~$k$ down to depth~$2$ yields
\[
  \Real_k(X) \simeq \Real_2(X)
\]
...
\begin{thm}[\texorpdfstring{Depth-two law for the specified $2$D toy discipline}{Depth-two law for the specified 2D toy discipline}]
...
\midrule
Chosen CCHM-style cubical core / Cubical Agda realization & yes; explicit swap obstruction, with clutching as a checked example & yes; horn reduction, stabilization, and recent-history factorization & yes; replacement interface and fixed raw bridge mechanized & yes \\
Other cubical variants & only if a binary witness is checked case by case & only if the horn-reduction, theorem-facing fiber, and window clauses of \autoref{thm:horn-interface} are verified & only if the canonical-trace bridge clauses of \autoref{thm:horn-interface} are verified & no \\
...
The formalization is organized around the fixed extension calculus of \autoref{sec:framework}, its horn-computational abstraction layer, its depth theorems, the induced recurrence law, the fixed raw bridge, the concrete \texttt{SMod} adequacy instance, the case-study audit, and the clutching construction. \autoref{tab:mechanization-status} first makes the trusted boundary local. The table uses five statuses. \emph{\texttt{checked-internal}} means the claim is proved inside the fixed \(\Cext\) calculus. \emph{\texttt{checked-actual-cubical}} means the claim is proved using the actual Cubical Agda partial-element/composition interface rather than a synthetic singleton encoding; when this status is qualified by \autoref{rem:agda-open-box-boundary}, Cubical Agda checks the \texttt{Partial}/\texttt{Sub} boundary package and canonical filling operations while the fully fibrant compatible-element presentation remains an explicit adequacy step. \emph{\texttt{checked-adequacy-instance}} means a specific surface calculus has a checked elaboration theorem into \(\Cext\). \emph{\texttt{semantic-assumption}} marks broad transfer from arbitrary cubical surface languages or foundations. \emph{\texttt{paper-level-interpretation}} marks prose-level readings whose checked counterpart is a more explicit counted-interface package.

The absence of local postulates is reported separately from these statuses. A postulate-free Agda development proves the internal statements encoded by its datatypes, records, and definitions; it does not by itself prove that those datatypes are an adequate semantics for arbitrary Cubical Agda programs or for every CCHM-style foundation. Semantic adequacy is a separate theorem, and the new modal instance is included to show one nontrivial bridge across that boundary. The itemized map after the table records the exact paper-to-Agda theorem correspondence.
...
\AgdaPath{higher_hit_payload.yaml} & no recurrence law & $0$ & higher constructor is payload & user-supplied higher HIT constructor, not structural trace \\
\bottomrule
\end{tabular}
\end{table}
...
presentation invariance. It also includes a separate higher-payload fixture:
the higher horn rows above are structural horn obligations marked derived,
whereas \AgdaPath{higher_hit_payload.yaml} is a user-supplied higher HIT
constructor counted as payload, with no primitive structural trace
contribution.
...
\midrule
\autoref{rem:semantic-adequacy-package} semantic interface package & \texttt{semantic-assumption} for broad systems; checked wrappers for the fixed package & \AgdaPath{Semantics/CubicalFoundation.agda}, \AgdaPath{Semantics/SealedExtension.agda}, \AgdaPath{Semantics/PrimitiveTrace.agda}, \AgdaPath{Semantics/RawAdequacy.agda}, \AgdaPath{Semantics/TraceNormalForm.agda}, \AgdaPath{Semantics/SemanticHornReduction.agda} & yes \\
\autoref{rem:bridge-target} fixed raw bridge package & \texttt{checked-internal} & \AgdaPath{Metatheory/RawStructuralSyntax.agda}, \AgdaPath{Metatheory/RawStructuralTyping.agda}, \AgdaPath{Metatheory/SurfaceNormalizationBridge.agda}, \AgdaPath{Metatheory/SurfaceToHornImage.agda}, \AgdaPath{Metatheory/CanonicalTelescope.agda}, \AgdaPath{Metatheory/TraceCostNormalForm.agda}, \AgdaPath{Metatheory/PresentationEquivalence.agda}, \AgdaPath{Metatheory/MuInvariance.agda} & n/a \\
\autoref{thm:actual-open-box-contractibility}, \autoref{lem:missing-face-openext}, \autoref{lem:openext-substitution-stability} & \texttt{checked-actual-cubical}; fibrant compatible-element reading by \autoref{rem:agda-open-box-boundary} & \AgdaPath{CubicalOpenBox/Base.agda}, \AgdaPath{CubicalOpenBox/FaceSystem.agda}, \AgdaPath{CubicalOpenBox/Extension.agda}, \AgdaPath{CubicalOpenBox/Contractible.agda}, \AgdaPath{CubicalOpenBox/Substitution.agda}, \AgdaPath{Metatheory/StructuralBoundary.agda}, \AgdaPath{Metatheory/HornOpenBox.agda}, \AgdaPath{Metatheory/HornElaboration.agda}, \AgdaPath{Metatheory/StructuralHornToOpenBox.agda} & yes \\
\autoref{thm:modal-surface-adequacy} & \texttt{checked-adequacy-instance} & \AgdaPath{Surface/Modal/Syntax.agda}, \AgdaPath{Surface/Modal/Typing.agda}, \AgdaPath{Surface/Modal/Routes.agda}, \AgdaPath{Surface/Modal/Elaboration.agda}, \AgdaPath{Surface/Modal/Normalization.agda}, \AgdaPath{Surface/Modal/Adequacy.agda} & no for \texttt{SMod} \\
\autoref{thm:computational-replacement} & \texttt{checked-internal} on explicit canonical trace presentations & \AgdaPath{Metatheory/ComputationalReplacement.agda}, \AgdaPath{Metatheory/MuInvariance.agda} & yes \\
\autoref{prop:transparent} & \texttt{checked-internal} & \AgdaPath{Metatheory/InterfaceCalculus.agda} & no \\
\autoref{thm:basis-choice} & \texttt{paper-level-interpretation}; counted active-basis counterpart checked & \AgdaPath{Metatheory/FiniteInterfaceBasis.agda}, \AgdaPath{Metatheory/ActiveBasisContract.agda} & counted interface \\
\autoref{thm:canonicity}, \autoref{thm:trace}, \autoref{cor:mu-delta} & \texttt{checked-internal} through the fixed raw bridge & \AgdaPath{Metatheory/CanonicityDensity.agda}, \AgdaPath{Metatheory/TracePrinciple.agda}, \AgdaPath{Metatheory/InterfaceCalculus.agda}, \AgdaPath{Metatheory/SurfaceNormalizationBridge.agda} & yes \\
\autoref{lem:arity-dimension}, \autoref{thm:extensional} & \texttt{checked-internal} & \AgdaPath{Metatheory/Obligations.agda}, \AgdaPath{Metatheory/Extensional.agda} & no \\
\autoref{lem:horn-reduction}, \autoref{lem:kan-horn-contract}, \autoref{thm:upper}, \autoref{cor:contractible-factor}, \autoref{cor:chrono-window} & \texttt{checked-internal} using H1 & \AgdaPath{Metatheory/KanSubsumption.agda}, \AgdaPath{Metatheory/UpperBound.agda}, \AgdaPath{Metatheory/ChronologicalWindow.agda} & no \\
\autoref{thm:factorization-complete}, \autoref{thm:higher-elim}, \autoref{lem:telescopic} & \texttt{checked-internal} through the fixed raw bridge, with replacement checked & \AgdaPath{Metatheory/KanSubsumption.agda}, \AgdaPath{Metatheory/UpperBound.agda}, \AgdaPath{Metatheory/ChronologicalWindow.agda}, \AgdaPath{Metatheory/ComputationalReplacement.agda}, \AgdaPath{Metatheory/SurfaceToHornImage.agda} & yes \\
\autoref{thm:horn-interface}, \autoref{cor:cubical-horn-interface} & \texttt{checked-internal}; external transfer remains semantic & \AgdaPath{Metatheory/KanSubsumption.agda}, \AgdaPath{Metatheory/UpperBound.agda}, \AgdaPath{Metatheory/ChronologicalWindow.agda}, \AgdaPath{Metatheory/ComputationalReplacement.agda}, \AgdaPath{Metatheory/SurfaceToHornImage.agda} & yes \\
Broad arbitrary Cubical Agda adequacy & \texttt{semantic-assumption} & n/a & open \\
\autoref{thm:adjunction}, \autoref{cor:d2}; \autoref{thm:clutching} as an example & \texttt{checked-internal} & \AgdaPath{Metatheory/AdjunctionBarrier.agda}, \AgdaPath{Geometry/Clutching.agda}, \AgdaPath{Metatheory/ExactDepth.agda} & no \\
\autoref{thm:2d-foundations}, \autoref{cor:2ltt-instantiation} & \texttt{checked-internal} & \AgdaPath{Metatheory/TwoDFoundations.agda}, \AgdaPath{Metatheory/TwoLTTFoundations.agda} & no \\
\autoref{thm:recurrence}, \autoref{cor:d1}, \autoref{cor:fibonacci} & \texttt{checked-internal}; Fibonacci is the full-coupling endpoint of sparse recurrence & \AgdaPath{Metatheory/UniversalRecurrence.agda}, \AgdaPath{Metatheory/SparseDependencyRecurrence.agda}, \AgdaPath{Metatheory/FullCouplingEnvelope.agda}, \AgdaPath{Semantics/FullCoupling.agda}, \AgdaPath{Semantics/ScalingRecurrence.agda}, \AgdaPath{Semantics/FibonacciScaling.agda}, \AgdaPath{Core/DepthOneAffine.agda}, \AgdaPath{Core/AffineRecurrence.agda} & yes \\
Barrier and concrete decomposition checks & \texttt{checked-internal} local checks & \AgdaPath{Saturation/AbstractionBarrier.agda}, \AgdaPath{Saturation/AbstractionBarrier9.agda}, \AgdaPath{Saturation/Decomposition.agda} & no \\
Case-study audit data & \texttt{checked-internal} and audited & \AgdaPath{CaseStudies/UniverseExtension.agda}, \AgdaPath{CaseStudies/GlobalModality.agda}, \AgdaPath{CaseStudies/PromotedInterface.agda}, \AgdaPath{CaseStudies/SparseDatatype.agda}, \AgdaPath{CaseStudies/HigherPayload.agda}, \AgdaPath{scripts/coherence_depth_audit.py} & no \\
\autoref{cor:refactoring} & \texttt{checked-internal} & \AgdaPath{Metatheory/Refactoring.agda} & no \\
\bottomrule
\end{tabular}
...
\begin{quote}
\small
\textbf{Postulate-free is a syntactic audit result.}
In the table, ``no local postulates'' means that the listed local modules
typecheck under the stated Agda options without \texttt{postulate}
declarations or local \texttt{primitive} declarations beyond the trusted
Cubical Agda primitives imported from the library. A postulate-free proof of
\texttt{isContr SyntheticHorn} is evidence about \texttt{SyntheticHorn}; it is
not evidence about the actual CCHM/Cubical Agda horn-extension type unless the
synthetic type has first been identified with that actual type. The new
H0/H1 layer provides that identification for the structural horns used here.
\end{quote}
...
\item \autoref{rem:bridge-target} is mechanized for the fixed raw extension calculus. The syntax and admissibility boundary are in \AgdaPath{Metatheory/RawStructuralSyntax.agda} and \AgdaPath{Metatheory/RawStructuralTyping.agda} via \AgdaPath{rawStructuralClauseKind}, \AgdaPath{typed-role-kind-matches-raw}, and \AgdaPath{typed-role-support-depth-matches-raw}; normalization to canonical payload and trace signatures is in \AgdaPath{Metatheory/SurfaceNormalizationBridge.agda} via \AgdaPath{normalizeRawExtension}, \AgdaPath{raw-extension-elaborates-to-candidate}, \AgdaPath{raw-extension-normalizes-to-canonical-signature}, \AgdaPath{admissible-structural-trace-field-normalizes}, \AgdaPath{normalization-respects-presentation-equivalence}, and \AgdaPath{normalized-signature-matches-counted-interface}; and the raw-to-horn image theorem is in \AgdaPath{Metatheory/SurfaceToHornImage.agda} via \AgdaPath{surface-to-horn-normal-form}, \AgdaPath{higher-structural-fields-derived}, \AgdaPath{horn-image-complete-for-structural-clauses}, and \AgdaPath{raw-structural-normalizes-to-horn}.
\item The semantic parameters named in \autoref{rem:semantic-adequacy-package} are now explicit Agda records and wrappers. \AgdaPath{Semantics/CubicalFoundation.agda} packages \AgdaPath{SemanticCubicalFoundation}, \AgdaPath{KanStructure}, and optional \AgdaPath{UnivalenceStructure}; \AgdaPath{Semantics/SealedExtension.agda} packages \AgdaPath{SemanticLibraryState}, \AgdaPath{SemanticSealedLayer}, \AgdaPath{semantic-public-interface}, and \AgdaPath{semantic-transparent-zero-latency}; \AgdaPath{Semantics/PrimitiveTrace.agda} records \AgdaPath{traceHistoricalSupport} and \AgdaPath{TraceHistoricalArity}; \AgdaPath{Semantics/RawAdequacy.agda} packages \AgdaPath{RawAdequacyPackage}; and \AgdaPath{Test/SemanticDepthSmoke.agda} checks the depth/window wrappers against the existing cubical metatheory.
...
\item \autoref{thm:actual-open-box-contractibility} is checked up to the adequacy boundary of \autoref{rem:agda-open-box-boundary} by \AgdaPath{CubicalOpenBox/Base.agda}, \AgdaPath{CubicalOpenBox/Extension.agda}, and \AgdaPath{CubicalOpenBox/Contractible.agda} via \AgdaPath{StructuralOpenBox}, \AgdaPath{BoundaryFamily}, \AgdaPath{CompatibleBoundaryFamily}, \AgdaPath{CompatibleLid}, \AgdaPath{canonicalFillSub}, \AgdaPath{openExtCompatibleCanonicalFillAt}, \AgdaPath{OpenExt}, \AgdaPath{canonicalOpenExt}, \AgdaPath{isContrPathPSingleton}, and \AgdaPath{openExtIsContr}. \AgdaPath{CubicalOpenBox/FaceSystem.agda} records the optional finite side-system layer and its compiled \AgdaPath{Partial} boundary; \autoref{lem:openext-substitution-stability} is represented by the parametric reindexing API in \AgdaPath{CubicalOpenBox/Substitution.agda}. The missing-face bridge is now split between the earlier identity instance \AgdaPath{Metatheory/StructuralHornToOpenBox.agda} and the explicit adequacy package modules \AgdaPath{Metatheory/StructuralBoundary.agda}, \AgdaPath{Metatheory/HornOpenBox.agda}, and \AgdaPath{Metatheory/HornElaboration.agda}, where \AgdaPath{StructuralHornCubicalData} records the family, visible face, side boundary, overlaps, degenerate and transported faces, compatible base, center, decoding, endpoint/transport equations, and substitution-stability evidence consumed by \AgdaPath{structuralHornElaboration} to build a \AgdaPath{HornSemanticDerivation}. The generated cases are named by \AgdaPath{remoteComparisonHornElaboration}, \AgdaPath{degenerateHornElaboration}, and \AgdaPath{transportedHornElaboration}.
\item \autoref{thm:modal-surface-adequacy} is checked by the \AgdaPath{Surface/Modal} module group via \AgdaPath{modal-elaboration-sound}, \AgdaPath{modal-payload-preserved}, \AgdaPath{modal-primitive-trace-preserved}, \AgdaPath{modal-derived-horn-preserved}, \AgdaPath{modal-support-preserved}, \AgdaPath{modal-mu-preserved}, \AgdaPath{modal-explicit-horn-elaborates-through-HornElaboration}, \AgdaPath{modal-omitted-horn-inserts-derived-term}, \AgdaPath{modal-explicit-and-omitted-horn-same-derived-term}, and \AgdaPath{explicit-horn-presentation-invariant}. The smoke test \AgdaPath{Test/Surface/ModalAdequacySmoke.agda} checks the minimal, explicit-horn, omitted-horn, and rebundled presentations.
...
\item \autoref{thm:computational-replacement} is formalized in \AgdaPath{Metatheory/ComputationalReplacement.agda}. That module makes the canonical trace-signature interface explicit via \AgdaPath{CanonicalTraceSignature} and \AgdaPath{TracePresentation}, packages the presentation theorem itself as \AgdaPath{computational-replacement-preserves-canonical-presentation}, and exposes the higher-arity elimination consequence via \AgdaPath{higher-arity-fields-disappear-from-minimal-signature} and \AgdaPath{higher-arity-computational-replacement}. The theorem-facing raw-field deletion witness in \AgdaPath{Metatheory/ReplaceDerivedField.agda} now exposes \AgdaPath{forgetPrimitiveField}, \AgdaPath{insertReplacementTerm}, both round trips, and \AgdaPath{presentationEquiv}. \AgdaPath{Metatheory/MuInvariance.agda} connects this to \AgdaPath{mu} via \AgdaPath{computational-replacement-preserves-mu}.
...
\item \autoref{thm:higher-elim} is the surface-syntactic corollary of \AgdaPath{Metatheory/KanSubsumption.agda}, \AgdaPath{Metatheory/UpperBound.agda}, \AgdaPath{Metatheory/TwoDFoundations.agda}, \AgdaPath{Metatheory/DerivedTrace.agda}, \AgdaPath{Metatheory/HornElaboration.agda}, \AgdaPath{Metatheory/NormalizationDerived.agda}, \AgdaPath{Metatheory/ComputationalReplacement.agda}, and \AgdaPath{Metatheory/SurfaceToHornImage.agda}: the Agda development formalizes the horn-extension fiber, its canonical inhabitant, stabilization at depth~$2$, the abstract primitive-elimination law, the canonical-trace replacement interface, witnessed public status, dependency-ordered normalization, and the raw structural horn image via \AgdaPath{HornExtensionFiber}, \AgdaPath{structural-integration-horn-reduction}, \AgdaPath{remote-layer-obligation-derived}, \AgdaPath{structural-obligation-set-equivalence}, \AgdaPath{structural-stabilizes-at-two}, \AgdaPath{HornSemanticDerivation}, \AgdaPath{structuralHornElaboration}, \AgdaPath{PublicStatus}, \AgdaPath{NormalizedPublicField}, \AgdaPath{HigherStructuralDerivedStep}, \AgdaPath{higherStructuralTraceDerived}, \AgdaPath{normalizationClassifiesHigherStructuralByDerivation}, \AgdaPath{normalizeEliminatesHigherStructuralTrace}, \AgdaPath{normalizationAppliesDerivedFieldReplacement}, \AgdaPath{normalizationReplacementUsesOnlyLower}, \AgdaPath{DependencyOrderedNormalization}, \AgdaPath{normalizeTraceTelescopeInDependencyOrder}, \AgdaPath{normalizationTerminatesBecauseReplacementsUseLowerData}, \AgdaPath{normalizationMuCountsOnlyPayloadAndIrreducibleUnaryBinary}, \AgdaPath{NormalizationDerivedWitness}, \AgdaPath{normalizationDerivedWitness}, \AgdaPath{higher-arity-fields-disappear-from-minimal-signature}, \AgdaPath{higher-arity-computational-replacement}, \AgdaPath{higher-structural-fields-derived}, and \AgdaPath{higher-raw-structural-traces-derived}. The negative raw-shape guard \AgdaPath{Test/FakeHigherTraceNoBoundary.agda} records a high-arity trace-shaped value that has no structural-boundary package and therefore cannot become derived merely by carrying a horn role.
...
\item \autoref{thm:horn-interface} is the paper's stronger horn-computational wrapper. It combines the horn-reduction and contractible-factor package from \AgdaPath{Metatheory/KanSubsumption.agda} and \AgdaPath{Metatheory/UpperBound.agda}, the two-layer support factorization theorem \AgdaPath{primitive-obligations-factor-through-last-two} from \AgdaPath{Metatheory/ChronologicalWindow.agda}, the canonical-trace replacement interface of \AgdaPath{Metatheory/ComputationalReplacement.agda}, and the fixed raw bridge of \autoref{rem:bridge-target}. The semantic wrapper \AgdaPath{Semantics/SemanticHornReduction.agda} exposes \AgdaPath{HigherStructuralObligationSupport}, \AgdaPath{higher-horn-trace-derived-from-binary-boundary-data}, \AgdaPath{exported-horn-trace-computable-from-depth-two-data}, and \AgdaPath{semantic-derived-trace-respects-substitution}; \AgdaPath{Semantics/TraceNormalForm.agda} connects horn normal forms to explicit \AgdaPath{HornSemanticDerivation} data via \AgdaPath{semantic-trace-normal-form-with-derivation}. \autoref{cor:cubical-horn-interface} is the checked instantiation of that wrapper for the chosen cubical calculus.
\item \autoref{thm:upper} corresponds exactly to \AgdaPath{Metatheory/UpperBound.agda} via \AgdaPath{real-k-equivalent-real-k-minus-one}, \AgdaPath{real-k-equivalent-real-two}, \AgdaPath{structural-obligation-set-equivalence}, and \AgdaPath{structural-stabilizes-at-two}.
\item \autoref{cor:contractible-factor} corresponds exactly to \AgdaPath{Metatheory/KanSubsumption.agda} and \AgdaPath{Metatheory/UpperBound.agda} via \AgdaPath{depth-k-marginal-structural-obligation-open-ext}, \AgdaPath{depth-k-marginal-open-ext-contractible}, \AgdaPath{ContractibleRemoteFactor}, \AgdaPath{structural-obligation-contractible-factorization}, and \AgdaPath{contractible-remote-factor-contractible}; this is the formal trivialization of each higher exact realized obligation object into the depth-two boundary together with a canonically contractible remote horn chain.
...
\item \autoref{thm:adjunction} corresponds exactly to \AgdaPath{Metatheory/AdjunctionBarrier.agda} via \AgdaPath{explicit-binary-sealing-obstruction}. The semantic lower-bound wrapper also exposes the reusable swap components \AgdaPath{two-point-type-witness}, \AgdaPath{nontrivial-swap-equivalence}, \AgdaPath{swap-path-from-univalence}, \AgdaPath{transport-unary-clause-along-swap-changes-action}, and \AgdaPath{binary-comparison-between-refl-and-swap-not-unary-derived}.
...
\item \autoref{thm:recurrence} corresponds to \AgdaPath{Metatheory/UniversalRecurrence.agda} via \AgdaPath{WindowedRecurrenceContext}, \AgdaPath{ChronologicalRecurrenceContext}, \AgdaPath{UniversalAffineRecurrence}, \AgdaPath{universal-affine-recurrence}, \AgdaPath{universal-affine-recurrence-from-coherence}, \AgdaPath{semantic-depth-two-windowed-recurrence-context}, and \AgdaPath{semantic-universal-affine-recurrence}, read through \autoref{cor:mu-delta} so that the paper's surface-syntactic recurrence is stated over~$\mu$. The sparse dependency refinement is formalized in \AgdaPath{Metatheory/SparseDependencyRecurrence.agda} via \AgdaPath{SparseFootprint}, \AgdaPath{FullFootprint}, \AgdaPath{footprint-window-bound}, \AgdaPath{sparse-windowed-recurrence}, \AgdaPath{transparent-growth-zero-footprint}, and \AgdaPath{orthogonal-extension-below-full-envelope}; the semantic full-coupling wrapper includes \AgdaPath{semantic-full-coupling-window-iso}, \AgdaPath{semantic-full-coupling-cardinality-envelope}, and \AgdaPath{full-coupling-affine-recurrence-relative-to-adequacy}.
...
\item \autoref{cor:fibonacci} corresponds to the fully coupled endpoint of the sparse dependency calculus. The endpoint itself is formalized in \AgdaPath{Metatheory/FullCouplingEnvelope.agda} via \AgdaPath{full-coupling-envelope}, \AgdaPath{full-coupling-envelope-is-full-footprint}, \AgdaPath{full-coupling-specializes-sparse-recurrence}, and \AgdaPath{full-coupling-depth-two-affine-law}, while the arithmetic specialization is in \AgdaPath{Core/AffineRecurrence.agda} via \AgdaPath{Delta-bootstrap}, \AgdaPath{payload-aware-bootstrap-step}, \AgdaPath{shifted-trace-fibonacci-step}, \AgdaPath{shifted-trace-fibonacci-closed}, and \AgdaPath{tau-bootstrap-closed}; the semantic wrapper exposes \AgdaPath{constant-payload-fibonacci-scaling-relative-to-adequacy} and \AgdaPath{constant-payload-fibonacci-closed-relative-to-adequacy}. The code's \AgdaPath{Delta}-named arithmetic sequence is the paper's $\mu$-sequence after the bridge identification of \autoref{cor:mu-delta}.
\item The local barrier and concrete decomposition checks are mechanized by \AgdaPath{Saturation/AbstractionBarrier.agda}, \AgdaPath{Saturation/AbstractionBarrier9.agda}, and \AgdaPath{Saturation/Decomposition.agda}. The barrier modules check that the step~8 and step~9 inherited obligations use the owning layer's opaque record interface rather than reopening deeper truncation internals; the decomposition module packages \AgdaPath{WindowDecomposition}, the concrete witnesses \AgdaPath{decomp-step3} through \AgdaPath{decomp-step8}, and \AgdaPath{recurrence-from-decomp} with the concrete corollaries \AgdaPath{recurrence-at-3} through \AgdaPath{recurrence-at-8}, deriving the displayed Fibonacci recurrence from the window decomposition rather than from the closed form for~$\Delta$.
...
\item The case-study audit is checked by \AgdaPath{CaseStudies/UniverseExtension.agda}, \AgdaPath{CaseStudies/GlobalModality.agda}, \AgdaPath{CaseStudies/PromotedInterface.agda}, \AgdaPath{CaseStudies/SparseDatatype.agda}, \AgdaPath{CaseStudies/HigherPayload.agda}, \AgdaPath{Test/SemanticFixtureSmoke.agda}, the YAML fixtures in \AgdaPath{runs/coherence_depth_case_studies/}, and \AgdaPath{scripts/coherence_depth_audit.py}. These cover full-coupling universe/global examples, a promoted active-basis interface, a transparent zero-footprint and zero-latency lemma extension, a sparse datatype whose one-site footprint stays below the three-site full envelope, a refactored universe presentation with the same~$\mu$, and a higher HIT constructor classified as payload rather than structural trace.
\end{itemize}
}
...
The formalization includes both direct cubical metatheory and a distilled extension calculus. The UIP collapse, horn-extension computation, the canonical-trace replacement interface, recent-history factorization chain, lower-bound obstruction, exact depth-two corollary, clutching-family example, and fixed raw bridge are mechanized against cubical, UIP, or explicit raw-calculus primitives. The canonicity, trace, refactoring, recurrence, active-basis, sparse/full coupling, case-study, and abstract $2$D-foundations layers then package those results as the sealed-interface and counting calculus studied in the paper.

\subsection{Barrier discipline and affine recurrence}

Its organizing invariant is that resolved layers are exported through opaque interfaces. At the level of proof architecture, this is the exact content of \autoref{thm:trace}, realized in Agda by \AgdaPath{integration-trace-principle}: once an obligation has been discharged, later constructions depend only on the exported schema and not on the hidden derivation tree that discharged it. In \AgdaPath{Saturation/AbstractionBarrier.agda} and \AgdaPath{Saturation/AbstractionBarrier9.agda}, this is checked locally for the transitions around steps~8 and~9 by showing that inherited obligations are discharged from the owning layer's opaque record interface alone, without reopening deeper internal derivations.
...
On the counting side, the Agda development makes the modeling discipline explicit. The active primitive interface is represented as the disjoint union of the two most recent schema sets, and the saturation step is stated separately rather than hidden in the syntax of imports. In the basis-family presentation used here, recurrence counts the well-defined common cardinality of the counted active interface and then reads that abstract count as minimal opaque cost via \autoref{cor:mu-delta}. The sparse recurrence layer records smaller dependency footprints as \AgdaPath{SparseFootprint} data, the full-coupling envelope records the \AgdaPath{FullFootprint} endpoint, and the semantic bridge turns the exact depth-two chronological-window theorem into \AgdaPath{semantic-universal-affine-recurrence}. The concrete decompositions in \AgdaPath{Saturation/Decomposition.agda} then record, step by step, how the next obligation set splits into contributions from the two most recent sealed layers: \AgdaPath{WindowDecomposition} packages the split, \AgdaPath{decomp-step3} through \AgdaPath{decomp-step8} instantiate it, and \AgdaPath{recurrence-from-decomp} proves the recurrence from those split counts rather than by unfolding the Fibonacci closed form for~$\Delta$.

\AgdaPath{Metatheory/UniversalRecurrence.agda} proves \AgdaPath{universal-affine-recurrence} and \AgdaPath{semantic-universal-affine-recurrence}; \AgdaPath{Metatheory/SparseDependencyRecurrence.agda} packages sparse dependency footprints; \AgdaPath{Metatheory/FullCouplingEnvelope.agda} identifies the fully coupled endpoint and the semantic full-coupling cardinality envelope; and \AgdaPath{Core/DepthOneAffine.agda} and \AgdaPath{Core/AffineRecurrence.agda} package the exact arithmetic specializations \AgdaPath{depth1-affine-growth}, \AgdaPath{Delta-depth1-closed}, \AgdaPath{tau-depth1-closed}, \AgdaPath{payload-aware-bootstrap-step}, \AgdaPath{shifted-trace-fibonacci-step}, \AgdaPath{shifted-trace-fibonacci-closed}, and \AgdaPath{tau-bootstrap-closed}. In particular, the full-coupling endpoint first gives the payload-aware depth-two recurrence
\[
  \mu_{n+1} = \mu_n + \mu_{n-1} + \kappa_n+\kappa_{n-1}.
\]
Under the additional constant-payload assumption $\kappa_n=c$, this becomes
$\mu_{n+1}=\mu_n+\mu_{n-1}+2c$; the arithmetic layer then defines the shifted
sequence $U_n=\mu_n+2c$ and proves that $U$ satisfies the homogeneous Fibonacci
recurrence. In the code base those arithmetic lemmas still carry
\AgdaPath{Delta}-names, because they are formalized on the abstract counting
sequence before the bridge identification with~$\mu$. The zero-payload
specialization is linked back to \AgdaPath{Core/Nat.agda}, while
\AgdaPath{ObligationGraph/Recurrence.agda} records the payload-free corollary
and \AgdaPath{Test/Fibonacci.agda} checks the concrete Fibonacci identities.
...
\texttt{primitive}, and \texttt{--safe} status. The repository includes CI
...
\subsection{Reviewer-facing FAQ}
\label{subsec:faq}

\paragraph{Does the theorem say higher paths vanish?}
No. It concerns primitive public structural trace at sealed boundaries, not
internal homotopy dimension.

\paragraph{Does it say Cubical Agda libraries grow by Fibonacci recurrence?}
No. The Fibonacci statement is the fully coupled constant-payload endpoint of
the sparse footprint theorem. Ordinary libraries are typically sparse.

\paragraph{Is $\mu$ a line count?}
No. It counts primitive public trace schemas in canonical normalized
signatures.

\paragraph{Does $\hcomp$ make every higher coherence contractible?}
No. The contractible factors are the structural horn-extension packages
generated by the fixed sealing calculus.

\paragraph{Is the Agda artifact a proof for arbitrary surface programs?}
No. It checks the fixed model, the raw bridge stated in
\autoref{rem:bridge-target}, and the theorem-facing trace-accounting package.
...
obligations collapse to depth~$1$. In the fixed cubical instance, exact
...
a two-layer chronological window. Full coupling is then the additional scaling
hypothesis that turns this window into an equality recurrence.
...
The broader statement that cubical foundations in general obey this law remains
a conjectural program. The paper proves a depth-two theorem for a fixed formal
sealed-extension calculus and proposes it as evidence for a broader cubical
depth-two conjecture. The present calculus supplies a checked theorem object, a
trace-accounting discipline, lower-bound witnesses, and an explicit adequacy
boundary; future work is to prove transfer theorems from richer cubical surface
calculi and concrete foundational extension mechanisms into this sealed model.
