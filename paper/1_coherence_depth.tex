...
\begin{quote}
In the fixed cubical extension calculus of this paper, primitive public structural
trace has exact coherence depth two. For fully coupled sealed sequences, the
resulting two-layer window gives the depth-two affine recurrence; under uniform
payload size, the shifted sequence is Fibonacci.
\end{quote}
...
\[
  \mu_{n+1} = \mu_n + \mu_{n-1} + \kappa_n + \kappa_{n-1}.
\]
Under the additional constant payload assumption $\kappa_n=c$, this specializes to the shifted Fibonacci recurrence of \autoref{cor:fibonacci}. Sparse and orthogonal footprints remain bounded or sparse regimes, not equality-style Fibonacci laws. Thus full coupling enters only at the counting theorem: the exact depth-two claim itself is a primitive-trace statement about admissible sealed structural extensions.
\end{quote}
...
primitive candidates, while a horn role must carry a
\AgdaPath{HornSemanticDerivation} producing the corresponding semantic derived
horn trace.
...
\midrule
\autoref{rem:semantic-adequacy-package} semantic interface package & checked as explicit theorem parameters and wrappers & \AgdaPath{Semantics/CubicalFoundation.agda}, \AgdaPath{Semantics/SealedExtension.agda}, \AgdaPath{Semantics/PrimitiveTrace.agda}, \AgdaPath{Semantics/RawAdequacy.agda}, \AgdaPath{Semantics/TraceNormalForm.agda}, \AgdaPath{Semantics/SemanticHornReduction.agda} & yes \\
...
\autoref{thm:recurrence}, \autoref{cor:d1}, \autoref{cor:fibonacci} & formalized; Fibonacci is the full-coupling endpoint of sparse recurrence & \AgdaPath{Metatheory/UniversalRecurrence.agda}, \AgdaPath{Metatheory/SparseDependencyRecurrence.agda}, \AgdaPath{Metatheory/FullCouplingEnvelope.agda}, \AgdaPath{Semantics/FullCoupling.agda}, \AgdaPath{Semantics/ScalingRecurrence.agda}, \AgdaPath{Semantics/FibonacciScaling.agda}, \AgdaPath{Core/DepthOneAffine.agda}, \AgdaPath{Core/AffineRecurrence.agda} & yes \\
Barrier and concrete decomposition checks & fully formalized local checks & \AgdaPath{Saturation/AbstractionBarrier.agda}, \AgdaPath{Saturation/AbstractionBarrier9.agda}, \AgdaPath{Saturation/Decomposition.agda} & no \\
Case-study audit data & fully formalized and audited & \AgdaPath{CaseStudies/UniverseExtension.agda}, \AgdaPath{CaseStudies/GlobalModality.agda}, \AgdaPath{CaseStudies/PromotedInterface.agda}, \AgdaPath{CaseStudies/SparseDatatype.agda}, \AgdaPath{CaseStudies/HigherPayload.agda}, \AgdaPath{scripts/coherence_depth_audit.py} & no \\
