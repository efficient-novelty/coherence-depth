...
\item The semantic parameters named in \autoref{rem:semantic-adequacy-package} are now explicit Agda records and wrappers. \AgdaPath{Semantics/CubicalFoundation.agda} packages \AgdaPath{SemanticCubicalFoundation}, \AgdaPath{KanStructure}, and optional \AgdaPath{UnivalenceStructure}; \AgdaPath{Semantics/SealedExtension.agda} packages \AgdaPath{SemanticLibraryState}, \AgdaPath{SemanticSealedLayer}, \AgdaPath{semantic-public-interface}, and \AgdaPath{semantic-transparent-zero-latency}; \AgdaPath{Semantics/RawAdequacy.agda} packages \AgdaPath{RawAdequacyPackage}; and \AgdaPath{Test/SemanticDepthSmoke.agda} checks the depth/window wrappers against the existing cubical metatheory.
