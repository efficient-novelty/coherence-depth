...
\begin{abstract}
We study framework extension over a fixed cubical object theory rather than over ``cubical type theory'' in the abstract. The theorem object is a cubical extension calculus built on a chosen CCHM-style cubical core with univalence and Kan composition, together with a discipline of globally acting opaque extensions whose public interfaces are counted only up to canonical telescope isomorphism. We call the least historical depth at which the normalized obligation sets cease to change, up to canonical equivalence, the \emph{obligation depth}~$d_{\mathrm{obl}}$; separately, the \emph{minimal-signature depth}~$d_\mu$ records which primitive historical trace arities survive in canonical $\mu$-minimal public form. Within this calculus we prove a payload-aware trace principle decomposing each sealed public interface into new core payload and resolved coherence trace, and we measure extension debt by the presentation-invariant minimal opaque cost~$\mu$. The cubical metatheory together with the fixed raw bridge isolated in \autoref{rem:bridge-target} yields a depth-two theorem stack: exact stabilization of normalized obligation sets at depth~$2$; elimination of every primitive historical field of arity at least~$3$ from $\mu$-minimal public signatures; recent-history factorization of every surviving binary primitive clause through the previous two normalized signatures; and a genuine binary obstruction showing that depth~$1$ does not suffice. The paper also isolates an abstract horn-computational interface strong enough to imply that exact-stabilization-plus-bridge pattern, and verifies that stronger package for the chosen CCHM-style cubical calculus. Abstracting that proof pattern yields a universal affine recurrence for minimal opaque cost. In extensional systems with UIP the obligation depth collapses to~$1$. In the fixed fully coupled cubical extension calculus studied here the obligation depth is~$2$, while higher-coherence elimination shows $d_\mu \leq 2$, so
\[
  \mu_{n+1} = \mu_n + \mu_{n-1} + 2c
\]
under constant payload size~$c$; after the shift $U_n=\mu_n+2c$, the sequence is Fibonacci. The accompanying Cubical Agda development formalizes the candidate-and-obligation calculus, the cubical exact upper- and lower-bound metatheory, the canonical-trace replacement step underlying $\mu$-minimal elimination, the fixed raw extension-calculus bridge to canonical horn-generated trace signatures, active-basis naturality, sparse/full recurrence, and the case-study audit; \autoref{sec:mechanization} records the exact paper-to-Agda map and the boundary of the mechanized bridge.
\end{abstract}
...
The Cubical Agda development described in \autoref{sec:mechanization} formalizes the abstract candidate-and-obligation calculus, the cubical exact upper- and lower-bound metatheory, the contractible-factor decomposition of higher exact fibers, the recent-history factorization chain, the canonical-trace replacement interface for $\mu$-minimal elimination, the fixed raw extension-calculus bridge of \autoref{rem:bridge-target}, and the recurrence packages underlying \autoref{thm:extensional}, \autoref{lem:horn-reduction}, \autoref{thm:upper}, \autoref{cor:contractible-factor}, \autoref{cor:chrono-window}, \autoref{thm:horn-interface}, \autoref{cor:cubical-horn-interface}, \autoref{thm:adjunction}, \autoref{thm:clutching}, \autoref{cor:d2}, \autoref{thm:2d-foundations}, \autoref{thm:recurrence}, \autoref{cor:d1}, \autoref{cor:fibonacci}, and \autoref{cor:refactoring}. The surface-syntactic readings of \autoref{thm:canonicity}, \autoref{thm:trace}, \autoref{thm:factorization-complete}, \autoref{thm:higher-elim}, \autoref{lem:telescopic}, and the $\mu$-based recurrence are obtained by combining those formal metatheorems with the mechanized bridge for admissible declarations in the fixed raw extension calculus. \autoref{sec:mechanization} records the exact Agda theorem names and the trusted boundary.
...
This is an elimination statement about $\mu$-minimal normalized public signatures, not a claim that the corresponding exact obligation types are absent from $\Ocal^{(k)}(X)$. The cubical operations synthesize the replacement witness; the replacement theorem \autoref{thm:computational-replacement} upgrades that witness to a presentation-equivalent minimal signature.
\end{thm}
...
\begin{thm}[Exact depth-two stabilization of obligation sets]
\label{thm:upper}
...
\begin{proof}
\autoref{thm:upper} provides the exact stabilization statement by showing the obligation-set equivalence $\Ocal^{(k)}(X) \simeq \Ocal^{(2)}(X)$ for all $k \geq 2$, and \autoref{thm:higher-elim} turns that equivalence into the syntactic statement that no $\mu$-minimal public signature needs primitive arity-$3$ data. \autoref{lem:telescopic} shows that this depth-two exact-stabilization statement is realized by a genuine two-layer chronological window rather than merely by a bound on cell dimension. \autoref{thm:adjunction} and \autoref{thm:clutching} provide lower bounds by exhibiting genuine depth-two phenomena that do not collapse in the univalent setting. Therefore the least stabilizing obligation depth is~$2$.
\end{proof}
...
\begin{enumerate}
  \item For every candidate~$X$, every $k \geq 3$, and every boundary $\partial : \Ocal^{(k-1)}(X)$, the depth-$k$ obligation over~$\partial$ is canonically represented by a horn-extension fiber
  \[
    H_k(\partial)
    :=
    \sum_{\gamma : \Gamma^{(k)}_{\mathrm{miss}}(\partial)}
  ...
  \[
    \Ocal^{(k)}(X)
    \simeq
    \sum_{\partial : \Ocal^{(k-1)}(X)} H_k(\partial).
  \]
...
\begin{proof}
By Property~(1), each passage from depth $k-1$ to depth~$k$ with $k \geq 3$ decomposes the exact obligation set into a depth-$(k-1)$ boundary together with one horn factor. Property~(2) makes that horn factor contractible, so the projection to the boundary is a canonical equivalence. Iterating from depth~$k$ down to depth~$2$ yields
\[
  \Ocal^{(k)}(X) \simeq \Ocal^{(2)}(X)
\]
...
The mechanism behind the recurrence is structural. In the formalization, \autoref{thm:canonicity} is read through \AgdaPath{global-admissibility-forces-maximal-interface-density} together with the fixed raw bridge of \autoref{rem:bridge-target}, \autoref{thm:trace} by \AgdaPath{integration-trace-principle}, and \autoref{cor:chrono-window} by \AgdaPath{two-layer-chronological-window}. Sealing cost is therefore identified with the size of the active basis interface, while resolved coherence data disappear as derivations but remain present in the public signature as trace fields. In the cubical case, the marginal higher-arity data are horn extensions rather than closed-sphere fillers; \AgdaPath{structural-stabilizes-at-two} and \AgdaPath{cubical-coherence-depth-exactly-two} make the exact stabilization statement precise, the replacement interface of \AgdaPath{Metatheory/ComputationalReplacement.agda} and the \AgdaPath{mu} invariance theorem of \AgdaPath{Metatheory/MuInvariance.agda} turn cubically synthesized higher witnesses into elimination from the $\mu$-minimal part of a canonical trace presentation, and \AgdaPath{primitive-obligations-factor-through-last-two} gives the recent-history factorization underlying the two-layer window. The explicit swap obstruction and the clutching family supply exact lower-bound witnesses. Under constant payload size the full-coupling endpoint \AgdaPath{full-coupling-depth-two-affine-law} specializes the sparse recurrence to the affine depth-two law, and \AgdaPath{Delta-bootstrap} gives the shifted Fibonacci corollary.
...
The Cubical Agda formalization therefore certifies the cubical metatheory, fixed raw bridge, active-basis package, sparse/full recurrence package, and audit examples on which the paper's extension-calculus theorem stack rests, rather than merely illustrating isolated ingredients. \autoref{sec:mechanization} records the exact paper-to-Agda map; representative formal counterparts are \AgdaPath{history-truncates-to-one} for the UIP collapse, \AgdaPath{structural-stabilizes-at-two} and \AgdaPath{cubical-coherence-depth-exactly-two} for the exact depth package, \AgdaPath{higher-arity-computational-replacement} for the canonical-trace replacement step, \AgdaPath{raw-structural-normalizes-to-horn} for the fixed raw bridge, \AgdaPath{primitive-obligations-factor-through-last-two} for the recent-history factorization core, \AgdaPath{cubical-depth-two-law-for-2d-foundations} for the abstract $2$D-foundations wrapper, and \AgdaPath{full-coupling-depth-two-affine-law} together with \AgdaPath{Delta-bootstrap} for the fully coupled shifted Fibonacci corollary. The trust assumptions are exactly the Agda/Cubical kernel, the declared Cubical library, the fixed raw extension-calculus boundary, and the arithmetic layer used by the development, with no postulates in the cited proof modules.
