...
\begin{abstract}
We study sealed, globally acting extensions of one fixed CCHM-style cubical core, not arbitrary Cubical Agda programs or ordinary transparent library growth. Public interfaces are exported opaquely and counted only after canonical telescope normalization. The paper separates the exact realized obligation objects $\Real_k(X)$ from the primitive public trace fields that survive in $\mu$-minimal normalized signatures. In this fixed calculus, cubical horn computation gives exact stabilization $\Real_k(X)\simeq\Real_2(X)$ for $k\geq2$, higher structural trace fields are eliminated from $\mu$-minimal public form, and a swap obstruction shows that unary data do not suffice in general. A separate recent-history theorem turns the depth-two bound into a genuine two-layer chronological window. Consequently, in the fully coupled regime, minimal opaque trace cost satisfies the payload-aware recurrence
\[
  \mu_{n+1} = \mu_n + \mu_{n-1} + \kappa_n+\kappa_{n-1}.
\]
Under constant payload size $\kappa_n=c$, the shifted sequence $U_n=\mu_n+2c$ is Fibonacci. The accompanying Cubical Agda development formalizes the candidate-and-obligation calculus, the fixed raw bridge to canonical trace signatures, active-basis naturality, sparse/full recurrence, and the case-study audit; \autoref{sec:mechanization} records the paper-to-Agda map and the trusted boundary.
\end{abstract}
...
In the sealed fully coupled extension calculus over the fixed CCHM-style core~$C$, exact realized obligations stabilize at depth~$2$, primitive trace fields in $\mu$-minimal normalized public signatures have arity at most~$2$, surviving primitive support has a two-layer chronological window, binary data are genuinely necessary in general, and minimal opaque cost satisfies
\[
  \mu_{n+1} = \mu_n + \mu_{n-1} + \kappa_n + \kappa_{n-1}.
\]
Under constant payload size $\kappa_n=c$, this is the shifted Fibonacci recurrence of \autoref{cor:fibonacci}.
\end{quote}
...
\subsection{The full-window affine recurrence}

\begin{thm}[Full-window affine recurrence for minimal opaque cost]
...
\]
Then the minimal opaque cost satisfies the payload-aware affine recurrence
\[
...
\begin{cor}[Depth-two full-window recurrence]
...
\texttt{b150186d2544e7efeddd31e5d14a8b9ecbb100f7}. The theorem-facing modules
...
The theorem is deliberately tied to the extension calculus of
\autoref{sec:framework}. A different cubical calculus, a different sealing
discipline, or a richer raw declaration grammar would require a separate
normalization theorem, raw bridge, horn-generation theorem, and chronological
window proof. The paper therefore gives a reproducible theorem about one fixed
calculus, not a transfer theorem for all cubical foundations.

The full-window recurrence is also not a model of ordinary library growth.
...
higher operations of a candidate are erased. Such structure may remain
mathematically substantial; it is simply not counted as a new primitive public
trace field unless it appears in the sealed integration interface.
...
metatheoretic invariant of a fixed sealed cubical extension calculus and keeps it
separate from the minimal-signature depth~$d_\mu$ governing primitive public
trace fields. In extensional settings with UIP, exact obligations collapse to
depth~$1$. In the fully coupled cubical calculus studied here, exact obligations
stabilize at depth~$2$, higher structural horn factors are eliminated from
$\mu$-minimal public syntax, and surviving primitive support has a two-layer
